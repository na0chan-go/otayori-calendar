...
AIエージェントによる開発では、リポジトリ直下の [AGENTS.md](AGENTS.md) と [省トークン作業ガイド](docs/10_token_efficient_workflow.md) を参照します。
...
| POST | `/api/extracted-events/restore-statuses` | 直前操作後の状態を検証し、draft・confirmed・ignoredの範囲で元の状態へ戻す |
| PATCH | `/api/extracted-events/:id` | 予定候補のタイトル・日付・時間・場所・持ち物・提出期限・説明を更新する |
